...
--------------------------------
if (matches[1]:lower() == 'sticker x' and not Clang) or (matches[1]:lower() == 'استیکر x' and Clang) and is_mod(msg) then
		local eq = URL.escape(matches[2])
		local w = "500"
		local h = "500"
		local txtsize = "100"
...
--------------------------------
if (matches[1]:lower() == 'photo x' and not Clang) or (matches[1]:lower() == 'عکس x' and Clang) and is_mod(msg) then
		local eq = URL.escape(matches[2])
		local w = "500"
		local h = "500"
		local txtsize = "100"
...
--------------------------------
    if (matches[1]:lower() == 'gif' and not Clang) or (matches[1]:lower() == 'گیف' and Clang) and is_mod(msg) then
		...
		local url = "https://assets.imgix.net/gif-examples/imgix-logo-ani.gif?blur=150&frame=1"..w.."&h="..h.."&fit=crop&txt="..eq.."&txtsize="..txtsize.."&txtclr="..txtclr.."&txtalign=middle,center&txtfont=generator%20Condensed%20Medium&mono=ff6598cc" 
		local receiver = msg.to.id
		local  file = download_to_file(url,'text.jpg')
		tdcli.sendPhoto(msg.to.id, 0, 0, 1, nil, file, msg_caption, dl_cb, nil)
	end
